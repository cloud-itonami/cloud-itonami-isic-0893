...
(deftest production-logging-enabled-from-phase-1
  (is (contains? (:writes (get phase/phases 1)) :log-production-record))
  (is (not (contains? (:writes (get phase/phases 0)) :log-production-record))))
...
(deftest phase-3-auto-commits-three-of-four-ops
  (testing ":flag-safety-concern is the only non-auto-eligible op at phase 3 -- always human sign-off"
    (is (= #{:log-production-record :schedule-maintenance :coordinate-shipment}
           (:auto (get phase/phases 3))))))

(deftest gate-hold-always-wins
  (is (= :hold (:disposition (phase/gate 3 {:op :log-production-record} :hold)))))

(deftest gate-escalates-a-clean-non-auto-write
  (is (= :escalate (:disposition (phase/gate 3 {:op :flag-safety-concern} :commit)))))

(deftest gate-holds-a-write-disabled-in-this-phase
  (is (= :hold (:disposition (phase/gate 0 {:op :log-production-record} :commit))))
  (is (= :phase-disabled (:reason (phase/gate 0 {:op :log-production-record} :commit)))))

(deftest gate-auto-commits-a-clean-auto-eligible-write-at-phase-3
  (is (= :commit (:disposition (phase/gate 3 {:op :log-production-record} :commit)))))
