(ns saltops.operation
  "OperationActor -- one coordination request = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor
  (SaltOpsAdvisor) is sealed into a single node (:advise); its proposal
  is ALWAYS routed through the SaltExtractionGovernor (:govern) and the
  ...
  human operator/shift supervisor. The approver resumes with
  `{:approval {:status :approved}}` (or :rejected)."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [saltops.advisor :as advisor]
            [saltops.governor :as governor]
...
(defn build
  ...
  `saltops.store/Store`).
  opts:
    :advisor      -- a `saltops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; SaltOpsAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; SaltExtractionGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
